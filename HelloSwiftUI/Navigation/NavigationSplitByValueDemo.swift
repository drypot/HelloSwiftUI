//
//  NavigationSplitByValueDemo.swift
//  HelloSwiftUI
//
...
struct NavigationSplitByValueDemo: View {
    ...
    var body: some View {
        NavigationSplitView {
            List(Self.products, selection: $selectedProduct) { product in
                NavigationLink(product.name, value: product)
            }
        } content: {
            if let product = selectedProduct {
                List(product.parts, selection: $selectedParts) { part in
